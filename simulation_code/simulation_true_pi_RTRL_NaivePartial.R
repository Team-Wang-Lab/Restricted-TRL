...
results_sim <- function(seed,K=5000){
  ...
  #Fit Tree-Base Treatment Regime Rule for Constrained population at Stage 2 
  ...
  pi.hat_constrain_all <- M.propen(A[A_res_all,2],
                            cbind(X[A_res_all,1],L[A_res_all],R[A_res_all,1]))
  pi.hat_constrain_all <- pi.hat_constrain_all/
                            apply(pi.hat_constrain_all[,c(1,2)],MARGIN=1,FUN=sum)
  pi.hat_constrain <- pi.hat_constrain_all[A[A_res_all,2]!=2L,c(1,2)]
  
  ##Construct tree model
  tree.2.constrain <- DTRtree(Y=Y_constrain,
                              A=A_constrain,
                              H=cbind(X_constrain,R_constrain[,1]),#Everyone's A1 is the same so we dont need that.
                              pis.hat=pi.hat_constrain,
                              mus.reg=mu.hat_constrain[,c(1,2)],
  ...
  pi.hat_unconstrain <- M.propen(A_unconstrain,
                                 cbind(X_unconstrain[,2],
                                       R_unconstrain[,1]))
  
  tree.2.unconstrain <- DTRtree(Y=Y_unconstrain,
                                A=A_unconstrain,
                                H=cbind(X_unconstrain,A[A_unres,1],R_unconstrain[,1]),
                                pis.hat=pi.hat_unconstrain,
                                m.method="AIPW",
                                mus.reg=mu.hat_unconstrain,
                                minsplit=20,lambda.pct=0.05,depth=3)
  
  #Predict Optimal Treatment for Constrained Data
  predict.2.constrain <- predict.DTR(tree.2.constrain, 
                                     cbind(X[A_res_all,],
                                           R[A_res_all,1]))
  predict.2.unconstrain <- predict.DTR(tree.2.unconstrain, 
                                       cbind(X_unconstrain,A[A_unres,1],R_unconstrain[,1]))
  ...
  ###########Naive Method####################################################################################
  # Correct restricted patient records in training data, but no stage-wise restrictions.
  res_route<-(!(A[,1]==1L&A[,2]==2L))
  pi.hat_n <- M.propen(A[res_route,2],
                       cbind(X[res_route,],
                             R[res_route,1],(L.new[res_route]==1)))
  A_n <- as.vector(A[res_route,2])
  R_n <- R[res_route,]
  Y_n <- as.vector(R_n[,2])
  mu.hat_n <- mu.hat[res_route,]
  
  tree.2.naive <- DTRtree(Y=Y_n,A=A_n,
                          H=cbind(X[res_route,],R_n[,1],A[res_route,1]),
                          pis.hat=pi.hat_n,m.method="AIPW",
                          mus.reg=mu.hat_n,minsplit=20,lambda.pct=0.05,depth=3)
  
  #prediction for viable patient records
  predict.2.naive <- predict.DTR(tree.2.naive, 
                                 cbind(X[res_route,],
                                       R[res_route,1],
                                       A[res_route,1]))
  
  RF.2_n <-randomForest(R2[res_route]~.,
                        data=data.frame(A2=A2[res_route],
                                        X=X[res_route,],
                                        A1=A1[res_route],
                                        R1=R1[res_route]))
  
  R1.pseudo.naive<-R1[res_route]+R2[res_route]+
    predict(RF.2_n,newdata=data.frame(A2=predict.2.naive,X=X[res_route,],
                                      A1=A1[res_route],R1=R1[res_route]))-
    predict(RF.2_n,newdata=data.frame(A2=A[res_route,2],X=X[res_route,],
                                      A1=A1[res_route],R1=R1[res_route]))
  ...
  tree.1 <- DTRtree(Y=R1.pseudo,A=A1.new,H=X.new,pis.hat=pi.hat.1,m.method="AIPW",
                    mus.reg=mus.reg.1,minsplit=20,lambda.pct=0.05,depth=3)
  predict.1 <- predict.DTR(tree.1,X.new)
  
  #Stage 1 tree Naive
  pi.hat.1_n <- M.propen(A1,cbind(X.new.naive[,1],X.new.naive[,2],L.new.naive==1))
  ...
  Ares_naive <- ifelse(A1.pred_naive==1&A2.pred_naive==2,1,0) # if recommend non-viable route
  ...
  print(s2/K_test)#Opt1+Opt2 are correctly predicted new method
  print(s4/K_test)#Opt1+Opt2 are correctly predicted naive method
...
aa <- results_sim(100,K=3000)

mean(temp1[[2]]$Ares_naive)
