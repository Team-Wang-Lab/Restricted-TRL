...
results_sim <- function(seed,K=5000){
  ...
  #Fit Tree-Base Treatment Regime Rule for Constrained Data at Stage 2(Does not count people with a1=1 a2=2 when fitting the tree)
  A_res <- (A[,1]==1L&A[,2]!=2L) 
  A_res_all <- (A[,1]==1L)
  X_constrain <- X[A_res,]
  ...
  pi.hat_constrain_all <- M.propen(A[A_res_all,2],cbind(exp(X[A_res_all,1]),L[A_res_all],R[A_res_all,1]))
  pi.hat_constrain_all <- pi.hat_constrain_all/apply(pi.hat_constrain_all[,c(1,2)],MARGIN=1,FUN=sum)
  ...
  tree.2.constrain <- DTRtree(Y=Y_constrain,A=A_constrain,H=cbind(X_constrain,R_constrain[,1]),
                              pis.hat=pi.hat_constrain,mus.reg=mu.hat_constrain[,c(1,2)],m.method="AIPW",minsplit=20,lambda.pct=0.05,depth=3)
  ...
  pi.hat_unconstrain <- M.propen(A_unconstrain,cbind(exp(X_unconstrain[,1]),R_unconstrain[,1]))
  
  tree.2.unconstrain <- DTRtree(Y=Y_unconstrain,A=A_unconstrain,H=cbind(X_unconstrain,A[A_unres,1],R_unconstrain[,1]),
                                pis.hat=pi.hat_unconstrain,m.method="AIPW",mus.reg=mu.hat_unconstrain,minsplit=20,lambda.pct=0.05,depth=3)
  ...
  pi.hat_n <- M.propen(A[res_route,2],cbind(exp(X[res_route,2]),X[res_route,1],R[res_route,1],(L.new[res_route]==1)))#Model to assign A2
  A_n <- as.vector(A[res_route,2])
  R_n <- R[res_route,]
  Y_n <- as.vector(R_n[,2])
  mu.hat_n <- mu.hat[res_route,]
  
  tree.2.naive <- DTRtree(Y=Y_n,A=A_n,H=cbind(X[res_route,],R_n[,1],A[res_route,1]),pis.hat=pi.hat_n,m.method="AIPW",mus.reg=mu.hat_n,minsplit=20,lambda.pct=0.05,depth=3)
  
  #prediction
  predict.2.naive <- predict.DTR(tree.2.naive, cbind(X[res_route,],R[res_route,1],A[res_route,1]))
  
  RF.2_n <-randomForest(R2[res_route]~.,data=data.frame(A2=A2[res_route],X=X[res_route,],A1=A1[res_route],R1=R1[res_route]))
  
  R1.pseudo_n<-R1[res_route]+R2[res_route]+
    predict(RF.2_n,newdata=data.frame(A2=predict.2.naive,X=X[res_route,],A1=A1[res_route],R1=R1[res_route]))-
    predict(RF.2_n,newdata=data.frame(A2=A[res_route,2],X=X[res_route,],A1=A1[res_route],R1=R1[res_route]))
  ...
  #Stage 1 tree Naive
  pi.hat.1_n <- M.propen(A1[res_route],cbind(X[res_route,1],exp(X[res_route,2]),L[res_route]==1))
  mus.reg.1_n <- Reg.mu(R1.pseudo_n,A1[res_route],cbind(X[res_route,],L[res_route]))[1]$mus.reg
  tree.1.naive <- DTRtree(Y=R1.pseudo_n,A=A1[res_route],H=X[res_route,],pis.hat=pi.hat.1_n,m.method="AIPW",mus.reg=mus.reg.1_n,minsplit=20,lambda.pct=0.05,depth=3)
  predict.1.naive <- predict.DTR(tree.1.naive,X[res_route,])
...
sd(p_results3000Y$Y.pred-p_results5000Y$Y.observe)
sd(p_results3000Y$Y.pred_naive-p_results5000Y$Y.observe)
...
mean(p_results5000Y$Y.true[p_results5000Y$Ares_naive==1]-p_results5000Y$Y.pred_naive[p_results5000Y$Ares_naive==1])
mean(p_results3000Y$Y.true[p_results3000Y$Ares_naive==1]-p_results3000Y$Y.pred_naive[p_results3000Y$Ares_naive==1])

mean(p_results5000Y$Y.true[p_results5000Y$Ares_naive==1]-p_results5000Y$Y.pred[p_results5000Y$Ares_naive==1])
mean(p_results3000Y$Y.true[p_results3000Y$Ares_naive==1]-p_results3000Y$Y.pred[p_results3000Y$Ares_naive==1])
