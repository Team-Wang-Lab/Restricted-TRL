...
results_sim <- function(seed,K=5000){
  ...
  #Fit Tree-Base Treatment Regime Rule for Constrained population at Stage 2 (Does not count people with a1=1 a2=2 when fitting the tree)
  ...
  ##Propensity model for stage 2 restricted route
  pi.hat_constrain_all <- M.propen(A[A_res_all,2],cbind(X[A_res_all,1],L[A_res_all],R[A_res_all,1]))
  ...
  tree.2.constrain <- DTRtree(Y=Y_constrain,A=A_constrain,H=cbind(X_constrain,R_constrain[,1]),#Everyone's A1 is the same so we dont need that.
  ...
  pi.hat_unconstrain <- M.propen(A_unconstrain,cbind(X_unconstrain[,2],R_unconstrain[,1]))
  ...
  #Train RandomForest to Estimate R2 (for counterfactual outcome correction)
  ...
  pi.hat_n <- M.propen(A[res_route,2],cbind(X[res_route,],R[res_route,1],(L.new[res_route]==1)))
  ...
  tree.2.naive <- DTRtree(Y=Y_n,A=A_n,H=cbind(X[res_route,],R_n[,1],A[res_route,1]),
                          pis.hat=pi.hat_n,m.method="AIPW",mus.reg=mu.hat_n,minsplit=20,lambda.pct=0.05,depth=3)
  ...
  pi.hat.1 <- M.propen(A1.new,cbind(X.new[,1],X.new[,2],(L.new==1)))
  mus.reg.1 <- Reg.mu(R1.pseudo,A1.new,cbind(X.new,X3.new))[1]$mus.reg 
  ...
  #Stage 1 tree Naive
  pi.hat.1_n <- M.propen(A1[res_route],cbind(X[res_route,1],X[res_route,2],L[res_route]==1))
